...
impl interpreter::Eval for variables::Assignment {
    fn eval(&self, env: utils::Shared<environment::Environment>) -> types::Type {
        ...
        self.varlist.iter().for_each(|exp| { fill_out_typevec(exp, env.clone(), &mut var_typevec) });
        self.explist.iter().for_each(|exp| { fill_out_typevec(exp, env.clone(), &mut exp_typevec) });
        ...
        while !var_typevec.is_empty() {
            ...
            match key {
                types::Type::Id(var_id) => env.borrow_mut().add_variable(var_id, value),
