...
#[test]
fn test_empty_lexer() {
    let mut iter = Parser::new("".to_owned());

    if let Some(ref _element) = iter.next() {
        unreachable!()
    }
}
...
#[test]
fn test_keywords() {
    let mut iter = Parser::new("and Hello or World while".to_owned());
...
#[test]
fn test_strings() {
    let mut iter = Parser::new(r#""Hello" "world""!""#.to_owned());
...
fn test_invalid_strings() {
    let mut iter = Parser::new(r#""Hello"#.to_owned());
...
#[test]
fn test_numbers() {
    let mut iter = Parser::new("3 43 42.42 777".to_owned());
...
fn test_rollback() {
    let mut iter = Parser::new("3 43 42.42 777".to_owned());
    let mut start_pos = iter.position();
