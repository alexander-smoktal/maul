...
fn test_numerical_for() {
    let (_val, env) = interpret_rule("y = 3 for i = 0, 10 do y = y + i end", rules::block);
    assert_eq!(env, r#"{"y": RefCell { value: Number(48.0) }}"#);

    let (_val, env) = interpret_rule("y = 3 for i = 9, -1, -1 do y = y + i end", rules::block);
    assert_eq!(env, r#"{"y": RefCell { value: Number(48.0) }}"#);
}
