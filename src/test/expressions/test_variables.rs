...
fn test_assignment() {
    ...
    assert_eq!(parse_string(r#"var1.data, var2["key"] = 7, false"#, rules::stat),
        r#"[Single(Assignment { varlist: [Indexing { object: Id("var1"), index: Id("data") }, Indexing { object: Id("var2"), index: String("key") }], explist: [Number(7.0), Boolean(false)] })]"#);
    assert_eq!(parse_string("var1, var2, var3[nil].func = -7, object:method(), 11 - 3 + 5", rules::stat),
        r#"[Single(Assignment { varlist: [Id("var1"), Id("var2"), Indexing { object: Indexing { object: Id("var3"), index: Nil }, index: Id("func") }], explist: [Unop(MINUS, Number(7.0)), Funcall { object: Id("object"), args: [], method: Some(Id("method")) }, Binop(PLUS, Binop(MINUS, Number(11.0), Number(3.0)), Number(5.0))] })]"#);
}
