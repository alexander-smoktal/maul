...
fn test_assignment() {
    assert_eq!(variables::parse_assignment(&mut make_lexer("one, two, three = 1, true, false")),
               Ok(exp!(util::Expressions(vec![
                   exp!(variables::Assignment(exp!(variables::Id(vec!["one".to_string()])), exp!(primitives::Number(1f64)))),
                   exp!(variables::Assignment(exp!(variables::Id(vec!["two".to_string()])), exp!(primitives::Boolean(true)))),
                   exp!(variables::Assignment(exp!(variables::Id(vec!["three".to_string()])), exp!(primitives::Boolean(false))))
               ])))
    )
}
