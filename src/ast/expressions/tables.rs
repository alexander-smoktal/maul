...
fn parse_field(lexer: &mut lexer::Lexer) -> ParseResult {
    ...
    if let Ok(expression) = lexer.parse_or_rollback(|lexer: &mut lexer::Lexer| {
        lexer
            .skip_expected_keyword(tokens::Keyword::LSBRACKET, "")
            .and_then(|_| lexer.parse_or_rollback(parse_exp))
            .and_then(|index| {
                lexer
                    .skip_expected_keyword(
                        tokens::Keyword::RSBRACKET,
                        "Expected ']' at the end of indexing",
                    )
                    .map(|_| index)
            })
            .and_then(|index| {
                lexer
                    .skip_expected_keyword(
                        ...
                        "Expected assignment after table indexing expression",
                    )
                    .map(|_| index)
            })
            .and_then(|index| {
    ...
    if let Some(id) = lexer.head().id() {
        if let Ok(expression) = lexer.parse_or_rollback(|lexer| {
            lexer.skip(1);
            lexer
                .skip_expected_keyword(
                    tokens::Keyword::ASSIGN,
                    "Expected assignment after table key expression",
                )
                .and_then(|_| {
                    lexer.parse_or_rollback(parse_exp).map(|value| {
...
pub fn parse_table_constructor(lexer: &mut lexer::Lexer) -> ParseResult {
    ...
    if let Ok(field) = parse_field(lexer) {
        ...
        loop {
            ...
            if let Ok(field) = parse_field(lexer) {
                ...
                match lexer.skip_expected_keyword(
    ...
    Err(error::Error::new(
        lexer.head(),
        "Expected fields inside table constructor expression",
    ))
}
